...
fn main () {
    ...
    let snapshot_dir = format!("{}/{}", std::env::current_dir().unwrap().to_str().unwrap(), ".snapshot/app_server3");
    let metadata_dir = format!("{}/{}", std::env::current_dir().unwrap().to_str().unwrap(), ".metadata/app_server3");
    let consensus: Arc<Mutex<raft::consensus::Consensus>> = raft::start(3, 9003, peers, state_machine, snapshot_dir, metadata_dir);
