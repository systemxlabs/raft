...
fn main () {
    ...
    let snapshot_dir = format!("{}/{}", std::env::current_dir().unwrap().to_str().unwrap(), ".data/app_server3");
    let consensus: Arc<Mutex<raft::consensus::Consensus>> = raft::start(3, 9003, peers, state_machine, snapshot_dir);
