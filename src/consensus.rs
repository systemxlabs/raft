...
impl Consensus {
    ...
    fn append_entries_to_peer(&mut self, peer_id: u64, heartbeat: bool) -> bool {
        let peer = match self.peer_manager.peer(peer_id) {
            ...
            None => {
                ...
                warn!(
                    "peer {} not found in peer_manager when appending entries",
                    peer_id
                );
                return false;
            }
        ...
        // 发送附加日志请求
        let resp = match self.tokio_runtime.block_on(
            self.rpc_client
                .append_entries(req, peer.server_addr.clone()),
        ) {
            Ok(resp) => resp,
            Err(_) => {
    ...
    fn request_vote(&mut self) {
        ...
        for peer_server_id in peer_server_ids.iter() {
            ...
            if let Ok(resp) = self
                .tokio_runtime
                .block_on(self.rpc_client.request_vote(req, peer.server_addr.clone()))
            {
                ...
                if resp.term > self.metadata.current_term {
                    info!(
                        ...
                        &peer.server_addr, &resp.term, self.metadata.current_term
                    );
    ...
    fn install_snapshot(&mut self, peer_id: u64) {
        ...
        loop {
            ...
            if let Err(e) = metadata_file.seek(std::io::SeekFrom::Start(offset)) {
                error!(
                    "install snapshot to {:?} failed to seek metadata_file, e: {}",
                    &peer.server_addr, e
                );
                break;
            }
            if let Err(e) = metadata_file.read_exact(&mut data) {
                error!(
                    "install snapshot to {:?} failed to read_exact metadata_file, e: {}",
                    &peer.server_addr, e
                );
                break;
            }
            ...
            match self.tokio_runtime.block_on(
                self.rpc_client
                    .install_snapshot(req, peer.server_addr.clone()),
            ) {
                Ok(resp) => {
                ...
                Err(e) => {
                    error!(
                        "install snapshot to {:?} failed to send rpc, e: {}",
                        &peer.server_addr, e
                    );
                    return;
                }
        ...
        loop {
            ...
            if let Err(e) = snapshot_file.seek(std::io::SeekFrom::Start(offset - metadata_size)) {
                error!(
                    "install snapshot to {:?} failed to seek, e: {}",
                    &peer.server_addr, e
                );
                break;
            }
            if let Err(e) = snapshot_file.read_exact(&mut data) {
                error!(
                    "install snapshot to {:?} failed to read_exact, e: {}",
                    &peer.server_addr, e
                );
                break;
            }
    ...
    fn step_down(&mut self, new_term: u64) {
        ...
        if new_term < self.metadata.current_term {
            error!(
                "step down failed because new term {} is less than current term {}",
                new_term, self.metadata.current_term
            );
            return;
        }
    ...
    fn become_leader(&mut self) {
        if self.state != State::Candidate {
            error!(
                "can't become leader because state is not candidate, current state is {:?}",
                self.state
            );
            return;
        }
        ...
        if let Err(e) = self.replicate(
            ...
            error!(
                "add noop entry failed after becoming leader, error: {:?}",
                e
            );
        }
    }
            ...
            match entry.r#type() {
                ...
                proto::EntryType::Configuration => {
                    ...
                    if configuration.is_configuration_old_new() {
                        info!(
                            "append Cnew entry when Cold,new commited, Cold,new: {:?}",
                            &configuration
                        );
        ...
        if configuration.is_configuration_old_new() {
            if commited {
                ...
                for server in configuration.new_servers.iter() {
                    if !self.peer_manager.contains(server.server_id)
                        && server.server_id != self.server_id
                    {
                        new_peers.push(peer::Peer::new(
                            server.server_id,
            ...
            None => {
                ...
                match old_new_configuration {
                    Some(old_new_configuration) => {
                        ...
                        match self
                            .replicate(proto::EntryType::Configuration, new_configuration.to_data())
                        {
                            Ok(_) => {
                                return true;
                            }
    ...
    pub fn handle_election_timeout(&mut self) {
        match self.state {
            ...
            State::Candidate => {
                ...
                self.metadata
                    .update_current_term(self.metadata.current_term + 1); // 任期加1
    ...
    pub fn handle_append_entries(
        &mut self,
        request: &proto::AppendEntriesReq,
        ...
        for configuration_entry in configuration_entries.iter() {
            self.apply_configuration(
                config::Configuration::from_data(configuration_entry.data.as_ref()),
                false,
            );
        }
    ...
    pub fn handle_request_vote(
        ...
        if request.term < self.metadata.current_term {
            info!(
                "Refuse vote for {} due to candidate's term is smaller",
        ...
        // 已投给其他candidate
        if self.metadata.voted_for != config::NONE_SERVER_ID
            && self.metadata.voted_for != request.candidate_id
        {
            info!(
                "Refuse vote for {} due to already voted for {}",
                request.candidate_id, self.metadata.voted_for
            );
            return refuse_reply;
        }
    ...
    pub fn handle_install_snapshot(
        &mut self,
        request: &proto::InstallSnapshotReq,
    ) -> proto::InstallSnapshotResp {
        // 比较任期
        if request.term < self.metadata.current_term {
            info!(
            ...
            return proto::InstallSnapshotResp {
                term: self.metadata.current_term,
            };
        }
    ...
    pub fn handle_get_leader(&mut self, request: &proto::GetLeaderReq) -> proto::GetLeaderResp {
        ...
        proto::GetLeaderResp { leader: None }
    }

    pub fn handle_get_configuration(
        &mut self,
        request: &proto::GetConfigurationReq,
    ) -> proto::GetConfigurationResp {
        ...
        let reply = proto::GetConfigurationResp { servers };
        reply
    }

    pub fn handle_set_configuration(
        &mut self,
        request: &proto::SetConfigurationReq,
        ...
        proto::SetConfigurationResp { success }
    }
}
